...
class CalendarHeader extends Component {
  ...
  render() {
    ...
    let months = []
    options.map((value) => {
      months.push(moment(this.props.month.toString('yyyy-MM')).subtract(-value, 'months').format('YYYY MMMM'))
    })
    return (
      <View
      ...
      >
        <View style={this.style.header}>
          ...
          <View style={{ flexDirection: 'row' }}>            
            <ModalDropdown           
              textStyle={this.style.monthText}
              dropdownStyle={{
                width: 200,
                height: 400,
                marginLeft: 10
              ...
              options={months}
              defaultValue={this.props.month.toString(this.props.monthFormat)}
              onSelect={(id) => this.props.addMonth(options[id])}
              onDropdownWillHide={()=>this.setState({monthVisiable:true})}
              onDropdownWillShow={()=>this.setState({monthVisiable:false})}
            >
            </ModalDropdown>
            {/* <View style={this.state.monthVisiable?this.style.dropDownVisible:this.style.dropDownInVisible}/> */}
            {indicator}
          </View>
          {rightArrow}
        </View>
